...
function encodeToRot13(str) {
    let init = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    let finish = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    let result = '';
    for (let i = 0; i < str.length; i++) {
      let buf = init.indexOf(str[i]);
      if (buf != -1) {result += finish[buf]} else {result += str[i]};
    }
    return result;
}
